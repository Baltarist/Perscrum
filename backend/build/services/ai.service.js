...
class AIService {
    ...
    static async checkUsageLimits(userId) {
        ...
        const limits = constants_1.AI_LIMITS[user.subscriptionTier];
        ...
        if (limits.daily !== -1 && user.aiUsageCount >= limits.daily) {
            throw new types_1.CustomError(`AI usage limit exceeded. ${user.subscriptionTier} plan allows ${limits.daily} requests per day`, 429);
        }
    }
    /**
     ...
     */
    static async generateTaskSuggestions(userId, request) {
    ...
    `.trim();
        try {
            ...
            const suggestions = responseText
                ...
                .filter((task) => task.length > 10) // Filter out very short suggestions
            ...
            throw new types_1.CustomError('Failed to generate AI task suggestions', 500);
        }
    }
    /**
     * AI Chat functionality - coaching and project assistance
     */
    static async processChat(userId, request) {
...
Respond as Koç, the AI Scrum coach:
    `.trim();
        try {
            ...
            throw new types_1.CustomError('Failed to process AI chat message', 500);
        }
    }
    /**
     * Generate subtask suggestions for a specific task
     */
    static async generateSubtaskSuggestions(userId, request) {
        ...
        if (!task) {
            throw new types_1.NotFoundError('Task not found or access denied');
        }
    ...
    `.trim();
        try {
            ...
            const suggestions = responseText
                ...
                .filter((line) => line.trim().startsWith('-'))
                .map((line) => line.replace(/^-\s*/, '').trim())
                .filter((subtask) => subtask.length > 5) // Filter out very short suggestions
            ...
            throw new types_1.CustomError('Failed to generate AI subtask suggestions', 500);
        }
    }
    /**
     * Analyze sprint for retrospective insights
     */
    static async analyzeSprintRetrospective(userId, sprintId) {
        console.log('📊 AI: Analyzing sprint retrospective for sprint:', sprintId);
        // Check usage limits
        await this.checkUsageLimits(userId);
        // Get sprint data
        const sprint = await database_1.default.sprint.findFirst({
            where: {
                id: sprintId,
                project: {
                    OR: [
        ...
        if (!sprint) {
            throw new types_1.NotFoundError('Sprint not found or access denied');
        }
...
- [Recommendation 3]
    `.trim();
        try {
            ...
            throw new types_1.CustomError('Failed to analyze sprint retrospective', 500);
        }
    }
    /**
